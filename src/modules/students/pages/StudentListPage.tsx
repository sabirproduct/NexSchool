...
export function StudentListPage() {
  ...
  const studentQRPayload = useCallback((student: Student): string => {
  ...
  const printStudentIdCard = async (student: Student) => {
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button
              ...
              onClick={() => {
                ...
                const headers = ['Full Name', 'Roll No', 'Class', 'Section', 'Gender', 'Mobile', 'Email', 'Status', 'Admission No', 'Admission Date'];
                const csvRows = rows.map(s => [
                  s.fullName,
                  s.academic?.rollNo ?? '',
                  s.academic?.classId ?? '',
                  s.academic?.sectionId ?? '',
                  s.gender,
                  s.mobile,
                  s.email ?? '',
                  s.status,
                  s.academic?.admissionNo ?? '',
                  s.academic?.admissionDate ?? '',
              ...
              className="rounded-2xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
            >
              Export CSV
            </button>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white shadow-sm">
        <StudentTable
          ...
          onDelete={async (id) => {
            await useStudentsStore.getState().remove(id);
          }}
        />
      </div>
